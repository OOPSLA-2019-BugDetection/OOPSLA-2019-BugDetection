...
def go_through_files(root, output_root):
    ...
    buggy_lines = json.load(buggy_line_file)
    check_list = {}
    long_path_list = []
    buggy_states_list = []
    for project_path, dirnames, filenames in os.walk(root):
        for filename in filenames:
            if filename.endswith('.java'):
                ...
                subprocess.call("java -jar ASTgenerate.jar "+ os.path.join(project_path, filename) + " " + os.path.join(output_root, "temp.txt"), shell=True)
                ...
                for method in long_paths:
                    ...
                    for i in range(len(long_path_set)):
                        ...
                        if int(line_cover[i][0]) in buggy_line_infile or int(line_cover[i][1]) in buggy_line_infile:
...
def prepare_data_files(path_num, path_token, check_list, long_path_list, buggy_states_list, output_root, w_size, w_window, w_workers, label):
    json.dump(check_list, os.path.join(output_root, "check_list.json"))
    word2vec_model = g.Word2Vec(long_path_list, size=w_size, window=w_window, workers=w_workers)
    word2vec_model.save_word2vec_format(os.path.join(output_root, "word2vec.txt"), binary=False)
    ...
    output_list = buggy_states_list.reshape([path_num, 1, 1])
    output_list = output_list.repeat(w_size, axis=2)
...
word2vec_size = cfg.get('localcontext', 'input_dim')
word2vec_window = cfg.get('localcontext', 'word_window')
word2vec_workers = cfg.get('localcontext', 'word_workers')
...
cfg.set('localcontext', 'input_length', path_t)
cfg.set('localcontext', 'output_dim', word2vec_size)
